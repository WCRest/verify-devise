...
RSpec.describe LockableUser, type: :model do
  describe "with a user with an verify id" do
    let(:user) { create(:lockable_verify_user) }
    ...
    describe "#invalid_verify_attempt!" do
      before(:all) {
        @old_lock_strategy = Devise.lock_strategy
        Devise.lock_strategy = :failed_attempts
      }
      ...
      it 'respects the maximum attempts configuration for Devise::Models::Lockable' do
        ...
        2.times { user.invalid_verify_attempt! }
      ...
      it "returns false until the account is locked" do
        Devise.maximum_attempts = 3
        2.times { expect(user.invalid_verify_attempt!).to be false }
        expect(user.invalid_verify_attempt!).to be true
      end
